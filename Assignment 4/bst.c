...
struct Node *_addNode(struct Node *cur, TYPE val)
{
	...
	//create new node pointer and assert the value enterred is not 0
	struct Node *newNode;
	assert(val != 0);

    //if not 0, we create the node
	if (cur == 0){
	...
	//run comparators to determine where to set the node
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	//loop until it hits bottom of the tree (null)
	while (current != 0){
        ...
        //if value less than current value, go left
        else if(compare(val, current->val) < 0){
            current = current->left;
        }

        //else go to the right if greater
        else{
...
TYPE _leftMost(struct Node *cur)
{
	...
	//loops until left child is null
	while(cur -> left != 0){
        cur = cur->left;
	}
...
struct Node *_removeLeftMost(struct Node *cur)
{
    ...
    //will free the left note and set it to the right child
	if(cur->left == 0){
        node = cur->right;
        free(cur);
        return node;
    ...
    //using recursion to find left most node.
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	if (compare(cur->val, val) == 0)
	{
        ...
        //swaps current with left most child
		cur->val = _leftMost(cur->right);

		//removes left most child and sets to right child
		cur->right = _removeLeftMost(cur->right);
	}

    //cycles to the left of the tree
	else if (compare(val, cur->val) == -1){
		cur->left = _removeNode(cur->left, val);
	}

    //cycles to the right of the tree
	else{
